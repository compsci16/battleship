...
describe("player", () => {
  ...
  it("throws error without board access", () => {
    ...
    expect(() => new Player(ownBoard)).toThrowError("No opponent board access");
  });

  it("receives passed board", () => {
    const ownBoard = new Gameboard(2);
    const oppBoard = new Gameboard(2);
    expect(new Player(ownBoard, oppBoard)).toMatchObject({
      ownBoard,
      oppBoard,
    });
  });
  ...
  it("can attack opponent grid", () => {
    ...
    const player1 = new Player(ownBoard, oppBoard);
    const player2 = new Player(oppBoard, ownBoard);
    player1.attack(0, 0);
